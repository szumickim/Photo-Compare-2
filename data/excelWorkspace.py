...
def create_summary_excel(products_collection, folder_to_save):
    summary_columns = [SummaryConst.PRODUCT_ID, SummaryConst.FIRST_PHOTO_ID, SummaryConst.FIRST_REFERENCE, SummaryConst.HEIGHT,
...
def add_column_to_excel(excel_path, products_collection, first_row):
    ...
    last_column = 1
    while sheet.cell(row=1, column=last_column).value and sheet.cell(row=1, column=last_column).value != "Similarity type":
        last_column += 1
    ...
    for product in products_collection:
        ...
        sheet.cell(row=start_row + i, column=last_column + 1).value = ";".join(f"{photos_pair.similarity_type},{photos_pair.photo1.name},{photos_pair.photo2.name}" for photos_pair in product.photos_pairs_list)
        i += 1
    # save the file
